---
id: "abvNmyQUJUk"
title: "7.5 - Forecasting Transformative AI from Biological Anchors with Ajeya Cotra"
url: "https://www.youtube.com/watch?v=abvNmyQUJUk"
channel: "AXRP"
channel_id: "UCIAzAR5dG7j_Kcvwjh7SQpg"
channel_url: "https://www.youtube.com/channel/UCIAzAR5dG7j_Kcvwjh7SQpg"
upload_date: "2021-05-28"
duration_seconds: 64
is_short: false
chapters: 0
transcript: {"source": "auto", "language": "en-orig"}
collections: ["channels/axrpodcast"]
retrieved: "2026-09-25"
---

# 7.5 - Forecasting Transformative AI from Biological Anchors with Ajeya Cotra

[Watch on YouTube](https://www.youtube.com/watch?v=abvNmyQUJUk) · AXRP · 2021-05-28 · 1:04

## Description

```text
If you want to shape the development and forecast the consequences of powerful AI technology, it's important to know when it might appear. In this episode, I talk to Ajeya Cotra about her draft report "Forecasting Transformative AI from Biological Anchors" which aims to build a probabilistic model to answer this question. We talk about a variety of topics, including the structure of the model, what the most important parts are to get right, how the estimates should shape our behaviour, and Ajeya's current work at Open Philanthropy and perspective on the AI x-risk landscape.

Unfortunately, there was a problem with the recording of our interview, so we weren't able to release it in audio form, but you can read a transcript of the whole conversation.

Link to the transcript: https://axrp.net/episode/2021/05/28/episode-7_5-forecasting-transformative-ai-ajeya-cotra.html
Link to the draft report "Forecasting Transformative AI from Biological Anchors": https://drive.google.com/drive/u/1/folders/15ArhEPZSTYU8f012bs6ehPS6-xmhtBPP
```

## Transcript

_Source: YouTube auto-generated captions (en-orig). Timestamps are [m:ss] from the start of the video._

[0:00] hi all this episode is somewhat unusual earlier this month i recorded an in-person interview with open philanthropies jayakotra about her draft report forecasting transformative ai from biological anchors this report develops a model to produce a probability distribution over the time when ai technology will be developed that transforms the world economy to a similar extent as the industrial revolution however due to my inexperience with in-person recording the audio turned out to be unusable that being said there's still a transcript that people can read in it we discussed the state of ai forecasting the structure of the model which parts were the most important to get right how these estimates should shape our behavior and ajay's current work of philanthropy and our perspective on the aix risk landscape as well as other topics we had over two hours of conversation so it was pretty wide-ranging a link to the transcript is available in the description and you can also find it at axrp.net i hope you give it a read and like the previous episode the costs of making this episode were covered by a grant from the long-term feature fund
